...
function Textarea({ label, ...props }: { label: string } & React.TextareaAttrib­uteHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <div className="space-y-2">
      <label className="block text-[11px] font-medium uppercase tracking-wider text-white/30">{label}</label>
      <div className="group rounded-xl border border-white/[0.06] bg-white/[0.02] p-px transition-all focus-within:border-[#7c6cf0]/30 focus-within:shadow-[0_0_20px_rgba(124,108,240,0.08)]">
        <textarea {...props} rows={3} className="w-full rounded-[11px] bg-transparent px-4 py-3 font-mono text-sm text-white/90 placeholder:text-white/15 outline-none resize-none" />
      </div>
    </div>
  );
}

function ClaimCard({ claim, claimId, onVote, onStartVoting, onResolve, walletAddress, isLoading }: {
    ...
    <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] overflow-hidden animate-fade-in-up">
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-center gap-4">
          <div><p className="text-[10px] text-white/25 uppercase tracking-wider">Claimant</p><p className="font-mono text-xs text-white/50 mt-0.5">{truncate(claim.claimant)}</p></div>
          <div><p className="text-[10px] text-white/25 uppercase tracking-wider">Coverage</p><p className="font-mono text-xs text-[#fbbf24]/70 mt-0.5">{formatAmount(claim.coverage_amount)} XLM</p></div>
          <div><p className="text-[10px] text-white/25 uppercase tracking-wider">Approvals</p><p className="font-mono text-xs text-[#34d399]/70 mt-0.5">{claim.approvals}</p></div>
          <div><p className="text-[10px] text-white/25 uppercase tracking-wider">Rejections</p><p className="font-mono text-xs text-[#f87171]/70 mt-0.5">{claim.rejections}</p></div>
        </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {claim.evidence.map((ev, i) => (
                <span key={i} className="inline-flex items-center gap-1 rounded-lg border border-white/[0.06] bg-white/[0.02] px-2.5 py-1 font-mono text-[10px] text-white/40"><FileTextIcon />{ev}</span>
              ))}
            </div>
          </div>
        )}
        <div className="flex gap-2 pt-1">
          {claim.status === "Filed" && (
            <button onClick={onStartVoting} disabled={isLoading || !walletAddress}
          ...
          {claim.status === "UnderReview" && onVote && (
            <>
              <button onClick={() => onVote(true)} disabled={isLoading || !walletAddress}
              ...
              <button onClick={() => onVote(false)} disabled={isLoading || !walletAddress}
              ...
              {onResolve && (
                <button onClick={onResolve} disabled={isLoading || !walletAddress}
...
export default function ContractUI({ walletAddress, onConnect, isConnecting }: ContractUIProps) {
  ...
  }, [walletAddress, category, desc, amount]);
          ...
          <div className="flex border-b border-white/[0.06] px-2">
            {tabs.map((t) => (
              <button key={t.key}
                ...
                className={cn("relative flex items-center gap-2 px-5 py-3.5 text-sm font-medium transition-all",
            ...
            {activeTab === "file" && (
              <div className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <div className="group rounded-xl border border-white/[0.06] bg-white/[0.02] p-px transition-all focus-within:border-[#7c6cf0]/30 focus-within:shadow-[0_0_20px_rgba(124,108,240,0.08)]">
                    <select
                      ...
                      className="w-full rounded-[11px] bg-[#0a0a0f] px-4 py-3 font-mono text-sm text-white/90 outline-none cursor-pointer"
                    >
                ...
                <div className="border-t border-white/[0.04] pt-5 space-y-4">
                  ...
                  <Input label="Claim ID" type="number" min="1" value={evidenceClaimId}
                    onChange={(e) => setEvidenceClaimId(e.target.value)} placeholder="Enter claim ID to add evidence" />
                  <Textarea label="Evidence Description" value={evidenceText}
                    onChange={(e) => setEvidenceText(e.target.value)} placeholder="e.g. Photos of car damage, Repair estimates..." />
                  {walletAddress ? (
                    <button onClick={handleAddEvidence} disabled={isAddingEvidence}
              ...
              <div className="space-y-5">
                ...
                <div className="flex gap-2">
                  <div className="flex-1 group rounded-xl border border-white/[0.06] bg-white/[0.02] p-px transition-all focus-within:border-[#4fc3f7]/30 focus-within:shadow-[0_0_20px_rgba(79,195,247,0.08)]">
                    <input type="number" min="1" value={browseId} onChange={(e) => setBrowseId(e.target.value)}
                      placeholder="Enter claim ID..." className="w-full rounded-[11px] bg-transparent px-4 py-3 font-mono text-sm text-white/90 placeholder:text-white/15 outline-none" />
                  </div>
                  <button onClick={handleBrowseClaim} disabled={isBrowsing}
                    ...
                    {isBrowsing ? <SpinnerIcon /> : <SearchIcon />} Lookup
                  </button>
                </div>
                {browseClaim && (
                  <ClaimCard claim={browseClaim.data} claimId={browseClaim.id} walletAddress={walletAddress}
                    onVote={handleVote} onStartVoting={handleStartVoting} onResolve={handleResolve}
              ...
              <div className="space-y-5">
                ...
                <div className="flex gap-2">
                  <div className="flex-1 group rounded-xl border border-white/[0.06] bg-white/[0.02] p-px transition-all focus-within:border-[#fbbf24]/30">
                    <input type="number" min="1" max="100" value={searchMyClaims}
                      onChange={(e) => setSearchMyClaims(e.target.value)}
                      placeholder="Max claims to load (default 20)..."
                      className="w-full rounded-[11px] bg-transparent px-4 py-3 font-mono text-sm text-white/90 placeholder:text-white/15 outline-none" />
                  </div>
                  <button onClick={handleLoadMyClaims} disabled={isLoadingMyClaims}
                    ...
                    {isLoadingMyClaims ? <SpinnerIcon /> : <RefreshIcon />} Load
                  </button>
                </div>
                {myClaims.length > 0 && (
                  <div className="space-y-3">
                    <p className="text-[10px] uppercase tracking-wider text-white/25">{myClaims.length} claim{myClaims.length !== 1 ? "s" : ""} found</p>
                    {myClaims.map(({ id, data }) => (
                      <ClaimCard key={id} claim={data} claimId={id} walletAddress={walletAddress} />
                    ))}
                  </div>
                )}
                {myClaims.length === 0 && !isLoadingMyClaims && (
                  <div className="flex flex-col items-center gap-3 rounded-xl border border-dashed border-white/[0.06] py-10 text-center">
                    ...
                    <p className="text-sm text-white/30">Click &ldquo;Load&rdquo; to browse community claims</p>
                    <p className="text-[10px] text-white/15">Permissionless — anyone can browse all claims</p>
                  </div>
                )}
              </div>
            )}
